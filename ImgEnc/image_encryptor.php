<?php
// Image Encryption Script using AES-GCM
// Author: Generated for ImgEnc project
...
class ImageEncryptor {
    ...
    public function decryptImage($encryptedPackage, $key) {
    ...
    public function generateShareLink($encryptedData, $originalFilename) {
        ...
        $_SESSION['encrypted_images'][$shareId] = [
            'data' => $encryptedData,
...
// Start session for storing shared images
session_start();
...
// Parse image path from URL path (after script name)
$imagePath = null;
...
if ($scriptPos !== false) {
    ...
    if (!empty($pathAfterScript) && !preg_match('/^[\?&]/', $pathAfterScript)) {
        $imagePath = urldecode($pathAfterScript);
    }
}

// Fallback to GET parameter if no path-based image found
if (!$imagePath) {
    $imagePath = isset($_GET['image']) ? $_GET['image'] : null;
}

// Convert forward slashes to backslashes for Windows paths
if ($imagePath && DIRECTORY_SEPARATOR === '\\') {
    $imagePath = str_replace('/', '\\', $imagePath);
}

if ($shareId) {
    // Handle shared link access
    if (isset($_SESSION['encrypted_images'][$shareId])) {
        $sharedData = $_SESSION['encrypted_images'][$shareId];
        ...
        $imageInfo = pathinfo($sharedData['filename']);
        $mimeType = 'image/' . strtolower($imageInfo['extension']);

        // Output the encrypted data as an image (this will appear corrupted)
        header('Content-Type: ' . $mimeType);
        header('Content-Length: ' . strlen($sharedData['data']));
        echo $sharedData['data'];
        exit;
    } else {
        die("Shared image not found or expired.");
    }
}
...
if ($imagePath) {
    try {
        ...
        if (isset($_GET['debug'])) {
            ...
            echo "Parsed imagePath: " . $imagePath . "<br>";
        ...
        // Encrypt the image
        $result = $encryptor->encryptImage($imagePath);
        ...
        <head>
            ...
            <title>Image Encrypted - ImgEnc</title>
            <style>
                ...
                .encrypted-image { max-width: 100%; border: 1px solid #ddd; margin: 10px 0; }
        ...
        <body>
            <div class="container">
                <h1>🔐 Image Encrypted Successfully</h1>

                <div class="result">
                    <h3>Encryption Details:</h3>
                    <div class="stats">
                        <strong>Original File:</strong> <?php echo htmlspecialchars($imagePath); ?><br>
                        ...
                        <p style="color: #e53e3e; font-size: 14px; margin: 5px 0;"><strong>⚠️ Save this key securely!</strong> You need it to decrypt the image.</p>
                    </div>

                    <h3>Encrypted Image (appears corrupted - same size as original):</h3>
                    <div style="background: #f5f5f5; padding: 15px; border: 1px solid #ddd; border-radius: 4px; margin: 10px 0;">
                        <p><strong>Visual Display:</strong> The encrypted binary data cannot be displayed as a valid image.</p>
                    ...
                    <p><small>This link contains the encrypted image data. Only someone with the decryption key can view the original image.</small></p>
                </div>

                <p><a href="<?php echo htmlspecialchars($_SERVER['SCRIPT_NAME']); ?>">← Encrypt Another Image</a></p>
            </div>
        </body>
        </html>
        <?php
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Image Encryptor - ImgEnc</title>
    <style>
        body { font-family: Arial, sans-serif; margin: 20px; background: #f5f5f5; }
...
<body>
    <div class="container">
        ...
        <p>Provide an image path directly to encrypt it using AES-256-CTR encryption</p>
        ...
        <div class="info">
            ...
            <ul class="feature-list">
                ...
                <li>✅ Direct image path input</li>
            </ul>

            <h3>Usage:</h3>
            <p>You can now use two methods to specify the image path:</p>

            <h4>Method 1: Path-based URL (Recommended)</h4>
            <div class="share-link">
                <code><?php echo htmlspecialchars($_SERVER['REQUEST_SCHEME'] . '://' . $_SERVER['HTTP_HOST'] . $_SERVER['SCRIPT_NAME']); ?>/path/to/your/image.jpg</code>
            </div>

            <h4>Method 2: Query parameter (Legacy)</h4>
            <div class="share-link">
                <code><?php echo htmlspecialchars($_SERVER['REQUEST_SCHEME'] . '://' . $_SERVER['HTTP_HOST'] . $_SERVER['SCRIPT_NAME']); ?>?image=path/to/your/image.jpg</code>
            </div>
        </div>
        ...
        <form method="get" class="upload-form">
            ...
            <input type="text" name="image" id="imagePath" placeholder="Enter full path to image file" style="width: 100%; padding: 10px; margin: 10px 0; border: 1px solid #ddd; border-radius: 4px; box-sizing: border-box;">
            <button type="submit" class="upload-btn" style="width: 100%;">Encrypt Image</button>
        </form>

        <div class="info">
            <h3>Quick Examples:</h3>
            <ul>
                <li><a href="test.jpg">test.jpg</a> (relative path)</li>
                <li><a href="C:/xampp/htdocs/ImgEnc/sample.png">C:/xampp/htdocs/ImgEnc/sample.png</a> (Windows absolute path - note forward slashes in URL)</li>
                <li><a href="/var/www/html/images/photo.gif">/var/www/html/images/photo.gif</a> (Linux absolute path)</li>
            </ul>
